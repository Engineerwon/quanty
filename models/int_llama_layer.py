...
class QuantLlamaAttention(nn.Module):
    ...
    def __init__(self, 
                 ...
                 args=None):
        super().__init__()
        self.config = config #config는 autoconfig로 불러온 건가?
        ...
        #로터리 포지셔널 인코딩을 처리하는 모듈. 원래의 org.module에서 복사해서 사용. copy.deepcopy를 사용해서 원본 모듈과 독립적으로 수정할 수 있는 복사본 생성
    ...
    def forward(
        ...
        bsz, q_len, _ = hidden_states.size() # 입력받은 hidden에서 배치사이즈와 문장길이를 받음

        # query_states = self.q_proj(hidden_states)
        # key_states = self.k_proj(hidden_states)
        # value_states = self.v_proj(hidden_states)
        #전부 양자화 레이어로 전환. 이후 멀티헤드 어텐션을 위해 텐서 shape을 변환
        query_states = self.q_proj(hidden_states).view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states =self.k_proj(hidden_states).view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        value_states = self.v_proj(hidden_states).view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)

        kv_seq_len = key_states.shape[-2] #past_key_value가 존재하면 과거의 키 길이를 더하여 최종 시퀀스 길이를 정함.
        if past_key_value is not None:
            kv_seq_len += past_key_value[0].shape[-2]
        cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len) #로터리 포지셔널 임베딩을 계산
        #계산된 cos, sin 인코딩을 query와 key에 적용.
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids) 


        # [bsz, nh, t, hd]
        #이전 단계의 key와 value를 현재 단계의 것과 결합하여 시퀀스 길이를 늘림. 캐시된 key와 value를 재사용하여 효율적으로 계산하기 위함.
        if past_key_value is not None:
            # reuse k, v, self_attention
            key_states = torch.cat([past_key_value[0], key_states], dim=2)
            value_states = torch.cat([past_key_value[1], value_states], dim=2)
        #use_cache가 True이면 현재의 key_states와 value_states를 캐시로 저장하여 반환.
        past_key_value = (key_states, value_states) if use_cache else None

        # repeat k/v heads if n_kv_heads < n_heads, key와 value를 필요에 따라 반복하여 num_heads를 맞춤. 필요한 경우에만
...
class QuantLlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        """
        residual = hidden_states #나중에 잔차연결하려고 다른 곳에 입력을 저장.
    ...
    def set_quant_state(self, weight_quant: bool = False, act_quant: bool = False):
        ...
        self.use_act_quant = act_quant
        names = []
        for name, m in self.named_modules(): #named_modules()는 nn.module의 클래스 메서드
            if isinstance(m, (QuantLinear, QuantMatMul)):
                names.append(name)
                m.set_quant_state(weight_quant, act_quant)
    ...
    def smooth_and_quant_temporary(self):
        if self.let:
            with torch.no_grad():
                for name, module in self.named_parameters(): #이것도 nn.module이 가진 클래스 메서드
    ...
    def let_parameters(self, use_shift=True):
        ...
        for n, m in self.named_parameters():
            if n.find(template) > -1: #.find()는 파이썬의 문자열 메서드. template가 문자열에 포함되어 있지 않으면 -1을 반환. 
                # 임시파라미터에 smooth나 smooth_scale로 이름을 붙여놓고 나중에 이 메서드로 찾아서 문자열로 반환하려고 하는 용도.
                params.append(m)
        return iter(params)  
    ...
    def omni_parameters(self, use_shift=True): #이건 위의 두 메서드를 한번에 합쳐 놓은 것.
        params = []
        template = "smooth" if use_shift else "smooth_scale"
        for n, m in self.named_parameters():
    ...
    def omni_state_dict(self, destination=None, prefix='', keep_vars=False): #omni parameter를 저장한 state dict를 만들기 위함. pytorch의 state_dict와 유사하다네?
        if destination is None:
            destination = OrderedDict() #orderdict는 그냥 dict와 유사하지만, 항목들이 '추가된 순서'를 기억한다.
        for name, param in self.named_parameters():
            if name.find('smooth') > -1 or name.find('bound_factor') > -1:
                destination[prefix + name] = param if keep_vars else param.detach() #keep_vars가 false면 detach()
        return destination
